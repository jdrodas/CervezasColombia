    ...
    public class EstiloRepository : IEstiloRepository
    {
        ...
        public async Task<List<Estilo>> GetAllEstilosAsync()
        {
            using (contextoDB.Conexion)
            {
                ...
                var resultadoEstilos = await contextoDB.Conexion.QueryAsync<Estilo>(sentenciaSQL, new DynamicParameters());
        ...
        public async Task<Estilo> GetEstiloAsync(int id)
        {
            ...
            using (contextoDB.Conexion)
            {
                ...
                string sentenciaSQL = "SELECT id, nombre FROM estilos " +
                    "WHERE id = @estilo_id " +
                    "ORDER BY nombre";

                var resultado = await contextoDB.Conexion.QueryAsync<Estilo>(sentenciaSQL, parametrosSentencia);
